...
<script>
  let see = {
    init: function () {
      ...
      $('#order_btn').click(() => {
        let cnt = Number($('#cnt').val());
        if (cnt <= 0) { alert('수량을 1 이상 선택해주세요.'); return false; }
        $('#order_qt').val(cnt);
      });
    }
  }
      ...
      <div class="col-md-6 product-info">
        ...
        <div class="mt-4 text-center">
          ...
          <div class="action-buttons">
            ...
            <form id="orderForm" method="post" action="/od/checkout">
              <input type="hidden" name="productId" value="${p.productId}">
              <input type="hidden" name="productName" value="${p.productName}">
              <input type="hidden" name="productPrice" value="${p.productPrice}">
              <input type="hidden" name="productQt" id="order_qt" value="1">
              <button type="submit" class="btn btn-info" id="order_btn">주문하기</button>
            </form>
